...
Goods and Services are not directly offered by other members of the community, instead, they are offered by Descentralized Autonomous Organizations, Service Nodes, through Contracts.

Inside Collective, all resources are owned by the Community and Service Nodes are established based on the Demands of this Community. To fulfill a contract, a Service Node will establish a set of Tasks and Resources needed. Tasks are fulfilled by the Members of the Community.

Every time a Task is fulfilled, it will pay the amount specified in the Contract to the Worker, and the same amount in Tokens for that particular Service Node. These Tokens will later be used as "voting power" when decisions need to be made in the Service Node. An optional delegation system can be used by a Worker to cast votes as another Worker.

To ensure the worker can execute a given Task, a Skill system was thought, where Tasks establish requirements and Workers are assessed by Bodies of Knowledge, specially designed Service Nodes that are responsible for Training, Improvement and Assessment of the Community Knowledge.

In the heart of the community lies the Common Fund, a particular Service Node responsible for funding Tasks in other Service Nodes. Since Service Nodes have no ownership and cannot profit, all direct funding of a Service Node will work as donations. There are many ways in which the Common Fund can have a solid reserve, it is always opened for donations, it can charge interest on the funding and require repayment and last but not least, it can Tax the system, either through fees collected on transactions or imposing a mandatory "Resource" on every Contract.
...
Workers not only have Value obtained from Tasks completed but also Tokens from all Service Nodes he worked with. Service Nodes may have additional information regarding a Worker, such as Reviews from Tasks executed or other issues identified. In order to take Tasks, Workers have to show that they have the required Skills. The certification process is controlled by a Service Node that regulates a group of activities, the Body of Knowledge.

Each Worker has a Balance, which is the sum of all Value he has obtained minus all Contracts he has consumed, it can be positive or negative and is unbounded (see The Common Fund for the How). If thought carefully, the Balance has very little importance except as to be kept positive symbolizing you are helping the Community thrive, since is the Community itself that decides the Value of the Work (through the Service Nodes) and therefore its importance.

Finally, since a Worker can only consume from a Service Node, there is no direct transfer of Value, and given the lack of private onwership, there is no inheritance in the system. Even though families still exist, the economic aspect is removed from it, leaving only the bonds and human aspects.
...
Since Collective removes ownership from the system, the Service Node cannot suppress Wage since it's controlled by the Workers themselves, and Tasks that are seen as unworthy will not find Workers and in result, the Service Node won't be able to meet the Demand and will either improve its Contracts or cease to exist and be replaced by another.
...
Skills are a way of establishing requirements for the execution of a Task, such as knowing how to Cook to take a Cooking Task for a Service Node Restaurant. It's the job of the Bodies of Knowledge to decide the granularity and specificity of skills, or how they relate with each other.

## The Service Node

These organizations represent modern day Companies, except that they are public owned and governed by those that have worked on it. The foundation of a Service Node requires an effort that grants its founders Tokens and as such, power over it, but unless they do all the work, this power will dilute as the business expands.

1) Service Node Balance - Since the Value gathering through Products being consumed and the spending through Resources and Tasks is not synchronized, the Service Node also has a Balance and it mostly can be Negative (only the stock effect may create positive Balance, but I'm not sure if that happens for other reasons and if it can grow indefinetly) but is bounded.

2) Service Node Creation - The problem here is how to decide which Service Nodes should be created. The Community may have to decide and maybe, one way is to have a Criteria to be met depending on the Resource being requested, so if a very scarce Resource is required, the Service Node is required to have a particular number of future consumers before it's allowed to operate.

Also, when one or more Workers create a Service Node, they receive Founder Tokens, a symbolic name given to the Tokens awarded to them that gives them equal share of the Service Node (for Voting).
